...
public interface TemplateMapper {
    ...
    //下面的是设备模板相关的mapper
    @Select("select * from equipment_template")
    List<EquipmentTemplate> getEquipmentInfo();

    @Select("select * from equipment_template where name like #{name}")
    List<EquipmentTemplate> selectEquipmentTemplateByName(@Param("name") String name);

    @Select("select * from equipment_template where equipment_id = #{equipment_id}")
    List<EquipmentTemplate> selectEquipmentTemplateById(@Param("equipment_id") int equipment_id);

    @Insert("insert into equipment_template(equipment_id, name, password, attribute,company_id) values(#{equipment_id},#{name},# {attribute},#{company_id})")
    Integer addEquipmentTemplate(EquipmentTemplate equipmentTemplate);

    @Delete("delete from equipment_template where name=#{name}")
    int deleteEquipmentTemplateByName(@Param("name") String name);
}
